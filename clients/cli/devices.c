                ...
                const char *bssid_user)
{
	...
	aps_all = nm_device_wifi_get_access_points (device);
	if (bssid_user) {
...
wifi_list_finish (WifiListData *data)
{
	...
	wifi_print_aps (data->wifi, data->nmc, data->bssid_user);
               ...
               gint64 rescan_cutoff)
{
	...
	if (needs_rescan) {
		...
		wifi_print_aps (wifi, nmc, bssid_user);
}
...
do_device_wifi_list (NmCli *nmc, int argc, char **argv)
{
	...
	for (i = 0; devices_all[i]; i++) {
		NMDevice *dev = devices_all[i];

		if (NM_IS_DEVICE_WIFI (dev))
			wifi_list_aps (NM_DEVICE_WIFI (dev), nmc, bssid_user, rescan_cutoff);
	}
